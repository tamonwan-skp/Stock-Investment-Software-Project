...
int main(){
	...
	float price[NUM_ROW],buy_sh,sell_sh,buy_pr,sell_pr,re_budget,budget = 10000,gain_loss,numyear,annual;
	int n_act,act_temp,num_sh = 0;
	int num_K = 14,num_D = 3,K_count;
	float H14, L14, C, K,K_b,D,D_b,temp_K[NUM_ROW],cur_price,temp_d,k_temp,Kmax = 0, Kmin = 5000;
...
//---------------------------------------strategy2--------------------------------------
	...
	cout <<"Date-Month-Year"<<"\tK(%)"<<"\tD(%)"<<"\tStatus"<<"\tsh pri"<<"\tNo.sh"<<"\tBalance"<<endl;
...
//findind initial %D
 	temp_d = 0;
  	for (i = NUM_ROW1; i < NUM_ROW1+num_D ; i++)
  	{
  		temp_d+=temp_K[i];
	}
	D_b = temp_d/num_D;
