...
int main(){
	...
	float buy_sh,sell_sh,buy_pr,sell_pr,re_budget,budget = 10000,gain_loss,fee = 10;
	int n_act,act_temp,num_sh = 0;
	float price[NUM_ROW],price_s[NUM_ROW],total1,avg1,total2,avg2,MA15,MA50,numyear,annual;
...
//-------------------------------------strategy1--------------------------------------------------------------
	...
	cout <<"Date-Month-Year"<<"\tMA[15]"<<"\tMA[50]"<<"\tStatus"<<"\tsh pri"<<"\tFee"<<"\tNo.sh"<<"\tBalance"<<endl;
    for (k = NUM_ROW1; k >= num1 ; k--)
    {
    	total1 = 0;
    	avg1 = 0;
        for (j = k; j < k+num1; j++)
